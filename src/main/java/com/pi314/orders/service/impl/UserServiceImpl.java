...
public class UserServiceImpl implements UserService {
    ...
    public User register(UserDTO userInfo) {
        User user = User.builder().email(userInfo.getEmail()).username(userInfo.getUsername()).appliedDiscount(0).active(false)
                .build();
    ...
    public void saveUser(User user) throws MessagingException, UnsupportedEncodingException {
        ...
        emailSenderService.sendActivateUserRequest(user);
    }

    @Override
    public void activateUser(Long id) throws MessagingException, UnsupportedEncodingException {
        User user = userRepository.findById(id).orElseThrow();
        user.setActive(true);
        userRepository.save(user);
        emailSenderService.sendUserIsActivatedEmail(user);
    }
